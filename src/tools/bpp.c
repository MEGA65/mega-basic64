...
// Use booleans to control those optimizations, for comparison purposes
bool shorten_variable_names = true;
...
void compact_line(char *l)
{
  ...
  for(i=0;l[i];i++) {
    ...
    if (shorten_variable_names) {
      if ((!quoteMode)&&(!commentMode)&&((!i)||(!isalpha(l[i-1])))) {
        if (isalpha(l[i])) {
    ...
    if (remove_spaces){
      if (l[i]==' '&&(!quoteMode)&&(!commentMode)) {
        // Remove spaces that aren't in strings
      } else {
        out[len++]=l[i];
      }
    } else {
      out[len++]=l[i];  // output the character in any case if not removing spaces
    }
    ...
    if (remove_comments){
      ...
      // Replace # and ' by rem, so that it can still be tokenized
      if (i&&(!quoteMode)&&(!commentMode)&&((!strncmp("#",&l[i+1],1))||(!strncmp("\'",&l[i+1],1)))) {
        commentMode=1;
        out[len++]='r'; out[len++]='e'; out[len++]='m'; out[len++]=' ';
        i=i+1;
      }
...
int main(int argc,char **argv)
{
  ...
  for(i=1;i<argc;i++) {
    ...
    while(line[0]) {
      ...
      if (line[0]=='\t' || (line[0]>='a' && line[0]<='z') || (line[0]=='#') || (line[0]=='\'')) {
        // Line starts with TAB, or a-z char, or comment char (if comments are kept), so allocate line number        
        line_number++;
  ...
  for(i=1;i<argc;i++) {
    ...
    while(line[0]) {
      ...
      for(j=0;line[j];j++) {
        if (line[j]=='\''){
          comment_mode=1; // still a comment after a second single-quote
        }
        if (
          (!j)
          && (
            (line[j]=='\t')
            || (
              (!(line[0]>='0' && line[0]<='9'))     // not a line number
              && (!(line[0]>='A' && line[0]<='Z'))  // not a label
              && !(line[j]=='\n')                   // not an empty line (CR/LF)
            )
          )
        ) {
